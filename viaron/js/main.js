;(function () {
    ...
    $(window).scroll(function() {
        ...
        var box1 = $('#box').offset().top;
        /*Если сделали скролл на 100px задаём новый класс для header*/
        if(box1 > 100){
            $('#box').addClass('phone');
        } else{
            /*Если меньше 100px удаляем класс для header*/
            $('#box').removeClass('phone');
        }
	...
	var clickMenu = function() {
		$('#fh5co-offcanvass a:not([class="external"])').click(function(){
			var section = $(this).data('nav-section');
		    $('html, body').animate({ scrollTop: $('[data-section="' + section + '"]').offset().top }, 500);
            // $('html, body').animate({ scrollTop: $('[data-section="' +this.hash.slice(1)+'"]').offset().top }, 1000 );
		    return false;
		});
    ...
    // var clickMenu = function() {
    //
    //     $('#fh5co-offcanvass a:not([class="external"])').click(function(event){
    //         var section = $(this).data('nav-section'),
    //             navbar = $('#fh5co-offcanvass');
    //
