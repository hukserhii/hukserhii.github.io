	...
	if($('input.datepicker').length) {
		$('input.datepicker').datepicker({ 
			...
			monthNames: [
				...
				$('input.date-month-12').val()],firstDay: 1});}
...
$('.hidden-caption').each(function() {
	...
	$('.modalWrap .wpcf7-textarea').removeAttr('cols').attr('rows', '5');
	$('#openForm').on('click', function(){
		if ($('.modalWrap').css('left') == '0px') {
			$('.modalWrap').css('left', '-385px');
			$('#openForm').html('<p class="_text">З а д а т ь<br><br> в о п р о с <i class="fa fa-envelope-o"></i></p>');
		} else {
		$('.modalWrap').css('left', '0px');
		$('#openForm').html('<p class="_text">З а к р ы т ь<br><br> ф о р м у <i class="fa fa-envelope-o"></i></p>');}})});